...
from password_manager import (
    init_db, verify_password, setup_password, reset_db, reset_failed_attempts, get_failed_attempts,
    add_entry, get_entry, list_entries, delete_entry, update_entry,
    update_password, export_json, import_json, generate_password, print_usage, DB_PATH, MAX_FAILED_ATTEMPTS
)
...
class PasswordManagerGUI:
    ...
    def do_login(self):
        ...
        if verify_password(pw):
            # Import and reset failed attempts on success
            from password_manager import reset_failed_attempts
            reset_failed_attempts()
            self.current_password = pw
            self.show_main_menu()
            self.set_status("[OK] Logged in")
        else:
            from password_manager import record_failed_attempt
            remaining = MAX_FAILED_ATTEMPTS - get_failed_attempts()
            deleted = record_failed_attempt()
            if deleted:
                messagebox.showerror("Security", 
                    f"Too many failed attempts ({MAX_FAILED_ATTEMPTS}).\nDatabase deleted for security.\n\nPlease restart the app.")
            else:
                messagebox.showerror("Error", f"Invalid password.\n{remaining} attempts remaining before database deletion.")
    ...
    def show_add_entry(self):
        ...
        ttk.Button(btn_frame, text="Save", command=self.do_add_entry).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(btn_frame, text="Back", command=self.show_main_menu).pack(side=tk.LEFT, padx=10, pady=5)
    ...
    def show_get_password(self):
        ...
        ttk.Button(btn_frame, text="Get", command=self.do_get_password).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(btn_frame, text="Back", command=self.show_main_menu).pack(side=tk.LEFT, padx=10, pady=5)
    ...
    def show_list(self):
        ...
        ttk.Label(search_frame, text="Search:").pack(side=tk.LEFT, padx=10, pady=5)
        ...
        # Category filter
        ttk.Label(search_frame, text="Category:").pack(side=tk.LEFT, padx=10, pady=5)
        self.category_var = tk.StringVar(value="All")
        self.category_combo = ttk.Combobox(search_frame, textvariable=self.category_var, 
                                           values=['All'], width=15, state='readonly')
        self.category_combo.pack(side=tk.LEFT, padx=10, pady=5)
        ...
        ttk.Button(btn_frame, text="Edit Selected", command=self.edit_selected_entry).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(btn_frame, text="Delete Selected", command=self.delete_selected_entry).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(btn_frame, text="Refresh", command=self.show_list).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(btn_frame, text="Back", command=self.show_main_menu).pack(side=tk.LEFT, padx=10, pady=5)
    ...
    def show_edit_entry(self, entry):
        ...
        # Password generator (after all fields)
        ...
        ttk.Button(btn_frame, text="Save Changes", command=self.do_edit_entry).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(btn_frame, text="Close", command=self.show_list).pack(side=tk.LEFT, padx=10, pady=5)
    ...
    def show_update(self):
        ...
        ttk.Button(btn_frame, text="Update", command=self.do_update).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(btn_frame, text="Back", command=self.show_main_menu).pack(side=tk.LEFT, padx=10, pady=5)
    ...
    def show_delete(self):
        ...
        ttk.Button(btn_frame, text="Delete", command=self.do_delete).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(btn_frame, text="Back", command=self.show_main_menu).pack(side=tk.LEFT, padx=10, pady=5)
