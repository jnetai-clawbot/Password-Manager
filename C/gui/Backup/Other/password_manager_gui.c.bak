...
/* Add entry with notes support - uses parameterized query to prevent buffer overflow */
...
static void on_add_save_clicked(GtkWidget *btn, gpointer data) {
    (void)btn;
    GtkWidget **entries = (GtkWidget**)data;
    if (!entries || !entries[0] || !entries[1] || !entries[2] || !entries[3]) {
        show_msg(GTK_MESSAGE_ERROR, "Error", "Internal error"); free(entries); return;
    }
    ...
    if (add_entry(site, user, pass, notes)) {
        free(entries);
        entries = NULL;
...
static void show_add_entry(void) {
    ...
    gtk_grid_attach(GTK_GRID(grid), gen_frame, 1, 4, 1, 1);
    ...
    GtkWidget **entries = malloc(sizeof(GtkWidget*) * 4);
...
static void on_remove_btn_clicked(GtkWidget *btn, gpointer data) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_YES) {
        if (delete_entry(site)) {
            /* Refresh will happen via the list being rebuilt */
        }
    }
    gtk_widget_destroy(dialog);
    g_free(site);
...
static void on_search_changed(GtkWidget *entry, gpointer data) {
    ...
    const char *filter = gtk_entry_get_text(GTK_ENTRY(entry));
    refresh_entries_list(list_container, filter);
}

static void on_clear_search_clicked(GtkWidget *btn, gpointer data) {
    GtkWidget **ctx = (GtkWidget**)data;
    GtkWidget *search_entry = ctx[0];
    GtkWidget *list_container = ctx[1];
...
static void refresh_entries_list(GtkWidget *list_container, const char *filter_text) {
    /* Use parameterized query for search - prevents SQL injection and handles search properly */
    sqlite3_stmt *stmt;
    ...
    if (search_len > 0) {
        ...
        sqlite3_bind_text(stmt, 1, filter_text, -1, SQLITE_STATIC);
    } else {
        if (sqlite3_prepare_v2(db,
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        fprintf(stderr, "DEBUG entry: site=%s, cat='%s'\n", site ? site : "(null)", cat ? cat : "(null)");
...
static void show_list_entries(void) {
    ...
    g_signal_connect(clear_btn, "clicked", G_CALLBACK(on_clear_search_clicked), clear_ctx);
...
static void show_edit_entry(const char *site) {
    ...
    /* Category row - plaintext, not encrypted */
    GtkWidget *category_entry = make_entry(FALSE);
    ...
    ctx[0] = (GtkWidget*)site;  /* Store original site name as pointer (cast) */
    ctx[1] = site_entry;
    ctx[2] = user_entry;
    ctx[3] = pass_entry;
    ctx[4] = notes_entry;
